...
export default class OrganizationPage extends BasePage {
    ...
    async uploadOrganizationImage(filePath: string) {
        ...
        await expect(this.organizationLocators.imageUploadSuccessMessage).toBeVisible();
    }
    ...
    getOrganizationRowByName(name: string) {
        return this.organizationLocators.organizationForm.filter({ hasText: name });
    }

    async verifyOrganizationCreated(name: string) {
        await expect(this.organizationLocators.alertMessage).toBeDefined();
        await expect(this.getOrganizationRowByName(name)).toBeVisible();
    }

    async editOrganization(name: string, updatedTrialDays: string) {
        await this.organizationLocators.editLocator.click();
        await this.fillTrialDays(updatedTrialDays)
    }

    async deleteOrganization(name: string) {
        await this.organizationLocators.deleteLocator.click();
        await this.Page.getByRole('button').getByText('Delete', { exact: true }).click();
    }
